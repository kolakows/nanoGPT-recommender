...
def pack_tokens(sequences, block_size, pad=True, pad_token=None):
    batch = []
    row = []
...
class ItemSeqDataset(Dataset):
    ...
    def __init__(self, sequences, block_size):
        ...
        self.packed_x = pack_tokens(X, block_size, pad_token=self.input_pad)
        self.packed_y = pack_tokens(Y, block_size, pad_token=self.target_pad)
        self.packed_seq_ids = pack_tokens(seq_ids, block_size, pad_token=self.seq_pad)
        self.packed_pos_ids = pack_tokens(pos_ids, block_size, pad_token=self.input_pad)
...
def collate_fn(batch):
    ...
    # x = torch.nested.nested_tensor(x, dtype=torch.long, layout=torch.jagged)
    # y = torch.nested.nested_tensor(y, dtype=torch.long, layout=torch.jagged)
    # # seq = torch.nested.nested_tensor(seq, dtype=torch.long, layout=torch.jagged)
    # # complex indexing in mask_mod doesn't work with jagged
    # seq = torch.tensor(seq, dtype=torch.long)
    # pos = torch.nested.nested_tensor(pos, dtype=torch.long, layout=torch.jagged)
    return x, y, seq, pos
